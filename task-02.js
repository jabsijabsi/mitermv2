...
const students = [
  ...
  function getSubjectsStatistics(students) {
    return students.map(student => {
      ...
      return {
        ...
        avarage: parseFloat(average.toFixed(2)),
      };
    });
  }
  
  function getHighAchievers(students, gradeThreshold) {
    return getSubjectsStatistics(students)
      .filter(student => student.avarage > gradeThreshold)
  ...
  function getMultiAchivers(students, threshold) {
    return students
      .map(student => {
        const expertSubjects = Object.values(student.grades).filter(grade => grade > threshold).length;
        return {
          name: `${student.firstName} ${student.lastName}`,
          expertSubjects,
        };
